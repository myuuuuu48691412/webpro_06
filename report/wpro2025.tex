...
\url{https://github.com/myuuuuu48691412/webpro_06.git}
...
\textbf{/saize/delete/:number} : 指定された番号のメニュー情報を配列から削除する．削除処理はデータを消去する重要な操作であり，誤操作や意図しない実行を防ぐ必要がある．そのため，本システムでは GET メソッドではなく POST メソッドを使用している．GET メソッドは URL へのアクセスだけで処理が実行される可能性があるが．POST メソッドはフォーム送信などの明示的な操作が必要なため，安全性が高い．また，HTML フォームが POST に対応している点も考慮した．誤操作防止の観点から GET ではなく POST メソッドを使用している．下記にわたるシステムも同様である．\\

また，本システムにおいて新規メニュー追加画面は，通常のHTMLファイルではなく，EJSを用いて実装している．EJSはHTMLの構造を保ちながら，サーバサイドでJavaScriptを埋め込むことができるテンプレートエンジンであり．Expressと組み合わせることで動的な画面生成が可能となる．本アプリケーションでは．一覧画面，詳細画面，編集画面，新規追加画面といったすべての画面をEJSで統一して実装している．これにより，画面構成や記述方法を統一でファイル管理が容易になるとともに，将来的に共通のヘッダやフッタを追加するなどの拡張にも対応しやすくなる．また，現時点では新規追加画面に動的な表示は含まれていないが，今後idの自動生成結果の表示や，入力エラー時のメッセージ表示，入力補助機能などを追加する場合，EJSを用いることで柔軟に対応できる．静的なHTMLではこのような拡張が困難であるため，テンプレートエンジンを採用することが適切である．
以上の理由から，本システムでは新規メニュー追加画面をHTMLファイルとして実装するのではなく，Expressの設計思想に沿ったEJSテンプレートとして実装している．下記にわたるシステムも同様である．
\subsubsection{遷移図}
...
\begin{lstlisting}
/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/instal/HEAD/install.sh)"
\end{lstlisting}
