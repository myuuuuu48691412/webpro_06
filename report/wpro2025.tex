...
\textbf{/saize/delete/:number} : 指定された番号のメニュー情報を配列から削除する．削除処理はデータを消去する重要な操作であり，誤操作や意図しない実行を防ぐ必要がある．そのため，本システムでは GET メソッドではなく POST メソッドを使用している．GET メソッドは URL へのアクセスだけで処理が実行される可能性があるが．POST メソッドはフォーム送信などの明示的な操作が必要なため，安全性が高い．また，HTML フォームが POST に対応している点も考慮した．誤操作防止の観点から GET ではなく POST メソッドを使用している．
